...
{ TreeNode * t = statement();
  ...
  (token!=ELSE) && (token!=UNTIL) && (token!=CASE) && 
  (token!=ENDSWITCH) && (token!=ENDWHILE))
  { TreeNode * q;
    q = statement();
    if (q!=NULL) {
      if (t==NULL) t = p = q;
...
{ TreeNode * t = NULL;
  switch (token) {
    case WHILE : t = while_stmt(); break;  //declaracao do While
...
TreeNode * while_stmt(void)  //adicionandoo WHILE
